...
def grouped_matmul_kernel(
    ...
    group_b_ptrs,
    group_c_ptrs,
    # device tensor of gemm ize. its shape is [group_size, 3]
    # dim 0 is group_size, dim 1 is the values of <M, N, K> of each gemm
    group_gemm_sizes,
    # device tensor of leading dimension size. its shape is [group_size, 3]
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        styles=[("blue", "-"), ("green", "-")],
        ylabel="runtime (ms)",
        plot_name="grouped_gemm_performance",
        args={},
    )
)
def benchmark(N, provider):
    ...
    return min_ms, max_ms, ms


benchmark.run(save_path='./08_grouped_gemm', print_data=False)
