...
def test_layer_norm(M, N, dtype, eps=1e-5, device='cuda'):
    ...
    assert torch.allclose(dx_tri, dx_ref, atol=1e-2)
    assert torch.allclose(dw_tri, dw_ref, atol=1e-2)
    assert torch.allclose(db_tri, db_ref, atol=1e-2)
...
bench_layer_norm.run(show_plots=True, print_data=True, save_path='./05_layer_norm')